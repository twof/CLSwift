...
class CLSwiftTests: XCTestCase {
    ...
    func testHappyPath() {
        let command = Command(triggers: ["hello"], help: "Says hello")
        let commandWitness = CommandWitnessFactory<Int>.mainWitness(command: command) { (params, state) in
        ...
        if let triggeredCommand = triggeredCommand {
            commandWitness.execute(commandCenter.input)
